...
4. The server validates every action against a strict whitelist (known action types, known
   widget/game IDs, sanity-checked CSS values) before it ever reaches the browser.
5. The frontend is a small action executor that applies the actions to the live page: updating CSS
   variables, starting canvas animations, or mounting a pre-built widget/game component.

The model never generates raw HTML/CSS/JS — it only *invokes* a fixed catalog of pre-built,
sandboxed widgets and games by name. This keeps the "AI redesigns the page" experience safe and
reliable instead of relying on brittle, arbitrary generated code.
...
- "let's play tic tac toe" / "I want to play snake"
...
the page (or restarting the server) replays the saved turns so your theme, chat log, and mounted
widgets/games come back automatically. Use the **Reset** button in the chat header to clear the
session and start over.

Note: widgets/games are remounted fresh on replay — their own internal state (typed todo items,
notes text, in-progress game boards) isn't currently captured, only the fact that they were opened.
...
server/
  ...
  systemPrompt.js      Builds the system prompt (action vocabulary + few-shot examples)
...
public/
  ...
  js/
    ...
    actions/             setTheme/animateElement, updateLayout/removeWidget, widget & game mounting
    widgets/              calculator, timer, todo, chart, notes
    games/                tictactoe, snake
```

## Extending it

To add a new widget or game:

1. Add a module under `public/js/widgets/` or `public/js/games/` exporting `{ title, mount(container, props) }`.
2. Register it in `public/js/actions/widgets.js`.
3. Add its id to `WIDGET_IDS` / `GAME_IDS` in `server/actionSchema.js` so the model is allowed to
   invoke it (the catalog is also surfaced to the model automatically via `systemPrompt.js`).
